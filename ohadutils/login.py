...
import shortuuid
import hashlib
su = shortuuid.ShortUUID(alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ234567")
email = getpass.getpass("Enter email: ")
password = getpass.getpass("Enter the password for encryption: ")
response = pyotp.totp.TOTP(su.uuid(password)).provisioning_uri(name=email, issuer_name='Ohadutils')
qr = qrcode.QRCode(
    version=2,
